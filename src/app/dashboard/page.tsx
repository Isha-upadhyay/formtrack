    ...
    <div className="p-6 md:p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-blue-700 transition hidden sm:block"
        >
          + New Form
        </Link>
      </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <Link
                  href="/onboarding"
                  className="border border-blue-400 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-500 transition"
                >
                  View Setup Guide
                </Link>
              </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Forms', value: totalForms, icon: '📋', href: '/dashboard/forms' },
          { label: 'Active Forms', value: activeForms, icon: '✅', href: '/dashboard/forms' },
          { label: 'Total Leads', value: totalLeads, icon: '👥', href: '/dashboard/leads' },
          { label: 'Leads Today', value: leadsToday, icon: '⚡', href: '/dashboard/leads' },
        ].map((stat) => (
          <Link key={stat.label} href={stat.href}>
            <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition cursor-pointer">
      ...
      {/* Charts — only show if there are leads */}
      {totalLeads > 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-10 text-center mb-6">
          <div className="text-4xl mb-3">📊</div>
          <p className="font-semibold text-gray-700 mb-1">No leads yet</p>
          <p className="text-sm text-gray-400 mb-4">Share your form link or embed it to start getting leads</p>
          <Link href="/dashboard/forms" className="text-blue-600 text-sm hover:underline">View your forms →</Link>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {forms.slice(0, 5).map((form) => (
              <div key={form.id} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50 transition">
                <div>
                  <p className="text-sm font-medium text-gray-900">{form.name}</p>
                  <p className="text-xs text-gray-400">Created {new Date(form.created_at).toLocaleDateString('en-IN')}</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Link href={`/dashboard/forms/${form.id}/edit`} className="text-xs text-blue-600 hover:underline">Edit</Link>
                </div>
              </div>
            ))}
          </div>
